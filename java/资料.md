...
- HashTable：线程安全的。
- TreeMap：实现了SortedMap接口，能够把它保存的记录根据键排序,默认是按键值的升序排序，也可以指定排序的比较器，当用Iterator遍历TreeMap时，得到的记录是排过序的。
- LinkedHashMap：LinkedHashMap有序，可分为插入顺序和访问顺序两种。
### Set
- HashSet：底层是HashMap，不能保证元素的顺序。允许null值。
- TreeSet：底层是TreeMap，可以保证元素是由顺序的。不允许null值。
